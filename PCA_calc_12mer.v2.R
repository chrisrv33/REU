...
write.table(per.var, "/rhome/cvaldez/bigdata/REU/results/pca.values/percentvariance12.txt", sep="\t", quote=F,row.names=F) # write table out

exp.var<-as.data.frame(cbind(1:10, summary(df.pca)[1,1:10])) #get values for exp. variance
colnames(exp.var)<-c("PC", "PercentVariance")
write.table (exp.var, "/rhome/cvaldez/bigdata/REU/results/pca.values/explainedvariance.txt", sep="\t", quote=F,row.names=F)


## get coords out
coord<-as.data.frame(df.pca$x) ## coordinates
write.table(coord,"/rhome/cvaldez/bigdata/REU/results/pca.values/coords12.txt", quote=F, sep="\t")

## get rotations out (used for loadings)
rot<-as.data.frame(df.pca$rotation)
write.table(rot, gzfile("/rhome/cvaldez/bigdata/REU/results/pca.values/loading12.txt.gz"), quote=F, sep="\t")

## get eigenvalues out
ev<-as.data.frame(df.pca$eigvals)
write.table(ev, gzfile("/rhome/cvaldez/bigdata/REU/results/pca.values/eigvals12.txt.gz"), quote=F, sep="\t")

# get sdev out
# sdev<-as.data.frame(df.pca$sdev)
# write.table(sdev, gzfile("/rhome/cvaldez/bigdata/REU/results/pca.values/sdev12.txt.gz"), quote=F, sep="\t")
