...
before<-read.table("O_sativa_14mers_raw.txt", dec=",")
gc.before<-before$V2 # only show GC Content values

# Upload normalized GC-corrected table
after<-read.table("O_sativa_14mers_GC_corr.txt", dec=",")
...
p<-ggplot(one, aes(x=GC, fill=Legend)) + geom_density(alpha=0.4) +  
  labs(title="14-mer GC content comparison",x="GC %", y = "Density") + theme_classic() + 
...
print(p)
ggsave("14mer GC comparison.png", p)
